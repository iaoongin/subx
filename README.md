...
- 支持订阅状态切换（启用/禁用）
- 实时订阅数量统计
- 支持订阅分组、分组切换与分组独立订阅地址
- 新建订阅后自动绑定到当前分组，也可绑定已有订阅
- 管理后台采用“左侧分组上下文 + 右侧订阅工作区”布局
- 订阅卡片默认展示链接 host，点击后复制完整链接
- 下拉控件统一使用自定义样式，不使用浏览器原生 `select`
- 响应式设计，完美适配移动端
...
1. 顶部 Hero 区
   ...
   - 包含“管理分组”“系统配置”“退出登录”
2. 统计区
   - 展示当前分组下的总订阅数、启用数、停用数
3. 左侧分组上下文侧栏
   - 负责分组选择、查看当前分组订阅地址、复制地址、预览订阅、刷新缓存/流量
   - 负责“新建订阅”“绑定已有订阅”这类分组内操作
4. 右侧订阅工作区
   - 展示当前分组内的订阅列表
   - 支持搜索、状态筛选、类型筛选
...
1. 先在左侧选择当前分组
2. 确认当前分组订阅地址和统计信息
3. 在当前分组下新建订阅，或绑定已有订阅
4. 在右侧列表中完成编辑、启停、删除、解绑等操作
5. 需要查看对外订阅效果时，使用“预览订阅”

### 管理订阅

1. **选择分组**
   - 进入后台后，先在左侧选择当前分组
   - 所有新增、绑定、刷新、预览操作都基于当前分组
...
- 分组与独立 Token 方案：`docs/specs/group-token.md`
- 管理后台 UI 重构方案：`docs/specs/admin-ui-workbench-redesign.md`
- 提交日志规范：`docs/specs/commit-message-convention.md`
...
3. 安装本地 Git hook (`pnpm hooks:install`)
4. 按提交日志规范提交更改，格式为 `type(scope): 摘要`
5. 可使用 `pnpm commitmsg:check -- .git/COMMIT_EDITMSG` 手动校验格式
6. 提交更改（例如：`git commit -m "feat(ui): 重构后台工作台布局"`）
...
- `scope` 可选，推荐使用模块名，如 `ui`、`groups`、`subscriptions`
- 摘要建议控制在 72 个字符以内
- 摘要末尾不要带句号
- 仓库已提供 `commit-msg` hook 进行本地校验
